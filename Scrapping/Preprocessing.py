...
skills = {'Data science': ['data science', 'Data science'],
          ...
          'Redis': ['Redis'],
          'Web Scrapping': ['Selenium', 'BeautifulSoup', 'Scipy', 'Scrap'],
...
def main():
    ...
    filenames = ['data/' + str.lower(job).replace(' ', '') + str.lower(country) + '.csv' for job, country in
                 zip(jobs, countrys)]
    ...
    data = pd.get_dummies(df, prefix=['Profession', 'Pays', 'Experience', 'Level'],
    ...
    data.to_csv('data/Prepo.csv', index=False, sep=';', encoding="utf-8-sig")
